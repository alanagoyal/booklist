...
-- Fast text-only search for books (no embedding required)
CREATE OR REPLACE FUNCTION public.text_search_books(
  search_query text,
...
BEGIN
  ...
  SELECT 
    ...
    b.description,
    b.genre,
    1.0 as similarity -- Perfect similarity for text matches
  FROM books b
  WHERE LOWER(b.title) LIKE LOWER('%' || search_query || '%')
     OR LOWER(b.author) LIKE LOWER('%' || search_query || '%')
     OR LOWER(b.description) LIKE LOWER('%' || search_query || '%')
  ORDER BY 
    CASE 
      WHEN LOWER(b.title) LIKE LOWER(search_query || '%') THEN 1
      WHEN LOWER(b.author) LIKE LOWER(search_query || '%') THEN 2
      WHEN LOWER(b.title) LIKE LOWER('%' || search_query || '%') THEN 3
      WHEN LOWER(b.author) LIKE LOWER('%' || search_query || '%') THEN 4
      ELSE 5
    END,
    b.title ASC
  LIMIT match_count;
...
-- Fast text-only search for people (no embedding required)
CREATE OR REPLACE FUNCTION public.text_search_people(
  search_query text,
  match_count integer DEFAULT 100
)
 RETURNS TABLE(id uuid, full_name text, type text, description text, url text, similarity double precision)
...
BEGIN
  ...
  SELECT 
    ...
    1.0 as similarity -- Perfect similarity for text matches
  FROM people p
  WHERE LOWER(p.full_name) LIKE LOWER('%' || search_query || '%')
     OR LOWER(p.type) LIKE LOWER('%' || search_query || '%')
     OR LOWER(p.description) LIKE LOWER('%' || search_query || '%')
  ORDER BY 
    CASE 
      WHEN LOWER(p.full_name) LIKE LOWER(search_query || '%') THEN 1
      WHEN LOWER(p.type) LIKE LOWER(search_query || '%') THEN 2
      WHEN LOWER(p.full_name) LIKE LOWER('%' || search_query || '%') THEN 3
      WHEN LOWER(p.type) LIKE LOWER('%' || search_query || '%') THEN 4
      ELSE 5
    END,
    p.full_name ASC
  LIMIT match_count;
